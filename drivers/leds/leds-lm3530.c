...
struct lm3530_als_mode_map {
	const char *mode;
	enum lm3530_als_mode mode_val;
};
static struct lm3530_als_mode_map als_mode_map[] = {
	{ "input_avrg", LM3530_INPUT_AVRG },
	{ "input_als1", LM3530_INPUT_ALS1 },
	{ "input_als2", LM3530_INPUT_ALS2 },
	{ "input_ceil", LM3530_INPUT_CEIL },
};
/**
 * struct lm3530_data
 ...
 * @ESD_task: task that checks if lm3530 is knocked out by ESD
 * @gen_config: initial value of config reg
 */
struct lm3530_data {
...
static int lm3530_get_als_mode_from_str(const char *str)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(als_mode_map); i++)
		if (sysfs_streq(str, als_mode_map[i].mode))
			return als_mode_map[i].mode_val;
...
#ifdef CONFIG_OF
static struct lm3530_platform_data __init *lm3530_led_probe_dt(
    struct i2c_client *client)
{
    ...
    if(of_property_read_string(np, "mode", &string))
        goto dts_error;
    pdata->mode = lm3530_get_mode_from_str(string);
    if(of_property_read_string(np, "als_input_mode", &string))
        goto dts_error;
    pdata->als_input_mode = lm3530_get_als_mode_from_str(string);
     
    if (of_property_read_u8(np, "max_current", &pdata->max_current))
        goto dts_error;
    if (of_property_read_bool(np, "pwm_pol_hi"))
        pdata->pwm_pol_hi = false;
    else
        pdata->pwm_pol_hi = true;
    
    if (of_property_read_u8(np, "als_avrg_time", &pdata->als_avrg_time))
        goto dts_error;
    if (of_property_read_bool(np, "brt_ramp_law"))
        pdata->brt_ramp_law = false;
    else
        pdata->brt_ramp_law = true;

    if (of_property_read_u8(np, "brt_ramp_fall", &pdata->brt_ramp_fall))
        goto dts_error;
    if (of_property_read_u8(np, "brt_ramp_rise", &pdata->brt_ramp_rise))
        goto dts_error;
    if (of_property_read_u8(np, "als1_resistor_sel", &pdata->als1_resistor_sel))
        goto dts_error;
    if (of_property_read_u8(np, "als2_resistor_sel", &pdata->als1_resistor_sel))
        goto dts_error;
    if (of_property_read_u32(np, "als_vmin", &pdata->als_vmin))
        goto dts_error;
    if (of_property_read_u32(np, "als_vmax", &pdata->als_vmax))
        goto dts_error;
    if (of_property_read_u8(np, "brt_val", &pdata->brt_val))
        goto dts_error;
//    if (of_property_read_u8(np, "pwm_data", &pdata->pwm_data))
//      goto dts_error;
 
    return pdata;
dts_error: 
    dev_err(&client->dev, "Device tree missing information");
    return ERR_PTR(-ENOSYS); //Dont know if right code
}
#endif //CONFIG_OF
static int ESD_thread(void *data)
{
	struct lm3530_data *drvdata = (struct lm3530_data *)data;
    int timeout=HZ + (HZ>>1); //1.5 seconds, spread the CPU load away from once every second
    int regval,err;
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (drvdata == NULL)
		return -ENOMEM;
    if(&client->dev.parent->of_node)
    {
        pdata = lm3530_led_probe_dt(client);
        if (IS_ERR(pdata))
        {
    ...
    drvdata->ESD_task = kthread_create(ESD_thread, (void*)drvdata, "ESD_task");
    if(IS_ERR(drvdata->ESD_task))
    { 
        printk("Unable to start kernel thread, ESD_task.\n");
        err = PTR_ERR(drvdata->ESD_task);
        drvdata->ESD_task = NULL;
        goto exit;
    }
    wake_up_process(drvdata->ESD_task); 
