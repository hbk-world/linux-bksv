...
int ci_hdrc_select_role(struct device *dev, enum ci_role role)
{
	int ret;
	struct ci_hdrc *ci = dev_get_drvdata(dev);

	if (ci->role == role) {
		// Specified role already active
		return 0;
	}
	...
	if (ci->usb_phy) {
		// ci_role_stop has disabled the PHY.
		// Wait a while before calling ci_role_start (which will re-enable the PHY).
		// 300 ms never seen working; 400 ms occasionally works;
		// 500 ms seems reliable; use 800 ms for additional stability
		msleep(800);
	}
...
EXPORT_SYMBOL_GPL(ci_hdrc_select_role);
