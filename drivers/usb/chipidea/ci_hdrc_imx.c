...
/* Returns non-zero if partner supports USB Battery Charging */
int ci_hdrc_bc_supported(struct device *dev)
{
	struct device *parent_dev = dev->parent;
	struct ci_hdrc_imx_data *data = dev_get_drvdata(parent_dev);;

	return (data->charger.max_current == 1500) ? 1 : 0;
}
EXPORT_SYMBOL_GPL(ci_hdrc_bc_supported);
