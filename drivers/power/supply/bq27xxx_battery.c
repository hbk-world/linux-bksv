 ...
 * http://www.ti.com/product/bq27220
...
/* BQ27220 diverent bit and registers */
#define BQ27220_FLAG_EDV2	    BIT(3)  // Battery voltage below EVD2 threshold, Low battery (resides in Operation-status-register)
#define BQ27220_FLAG_CFGUP	    BIT(10) // Config-update flag (resides in Operation-status-register)
#define BQ27220_FLAG_OTD	    BIT(10) // Overtemperature in charge condition
#define BQ27220_FLAG_OTC	    BIT(11) // Overtemperature in discharge condition 
#define BQ27220_FLAG_FD	        BIT(15) // Full-discharge detected
#define BQ27220_FLAG_SYSDWN	    BIT(1)  // System down. The system should shut down
#define BQ27220_FULLACCESS_BIT  (BIT(2) | BIT(1)) 
#define BQ27220_SEALED			0x30
#define BQ27220_SET_CFGUPDATE	0x90
#define BQ27220_GAUGE_STATUS	0x56
#define BQ27220_FULLACCESS_VAL	2  		
...
#define BQ27XXX_FULLACCESS_KEY0	0xFFFF
#define BQ27XXX_FULLACCESS_KEY1	0xFFFF
...
#define BQ27XXX_FULLACCESS_BIT  	BIT(14)
#define BQ27XXX_FULLACCESS_VAL		BIT(14)
...
enum bq27xxx_reg_index {
    ...
    BQ27XXX_REG_OS,     /* Operation Status */
...
static u8
	bq27000_regs[BQ27XXX_REG_MAX] = {
		...
		[BQ27XXX_REG_AP] = 0x24,
        [BQ27XXX_REG_OS] = INVALID_REG_ADDR,
...
// TODO: Init the below bq27220_dm_regs
static struct bq27xxx_dm_reg bq27220_dm_regs[] = {
	[BQ27XXX_DM_DESIGN_CAPACITY]   = { 0x9f, 0x92, 2, 0, 32767 },
...
static struct {
    ...
    [BQ27220]   = BQ27XXX_DATA(bq27220,   0x04143672, BQ27XXX_O_OTDC | BQ27XXX_O_CFGUP | BQ27XXX_O_RAM),
...
/* Purpose: For application that keeps track of battery cycle-count and wants
 *          to set this paramter. 
 *          (e.g. Some chip resets it to zero on a device with detachable battery)
 */
static int set_cyclecount = 0;
module_param(set_cyclecount, int, 0644);
MODULE_PARM_DESC(set_cyclecount,"For manual control of battery cycle count - 0 disables manual control");

/* Purpose: Force to update "di->charge_design_full". It is handy when data-memory
 *          is loaded from user space. This variable will be cleared after read.
 */
static bool update_design_cap = true;
module_param(update_design_cap, bool, 0644);
MODULE_PARM_DESC(update_design_cap,"Forces to re-read battery design capacity when polling (cleared after reading)");
			       ...
			       bool single)
{
    ...
    //dev_info(di->dev, "%s: di->regs[%d]=%#x single=%d readVal=%#x\n",__func__, reg_index, di->regs[reg_index], single, ret);
				...
				u16 value, bool single)
{
    ...
    //dev_info(di->dev, "%s: di->regs[%d]=%#x single=%d value=%#x\n",__func__, reg_index, di->regs[reg_index], single, value);
    
	return ret;
}


static int bq27xxx_poll_register(struct bq27xxx_device_info *di,
								int reg_index,					// Register index to poll
								u16 mask,						// Bit position of desired flag(s) 
								int expctVal,					// Expected value of flag(s)
								int timeout,					// timeout in ms.      0 => Do not poll
								int pollTime,					// Polling time in ms. 0 => Do not poll
								bool single)
{
    int ret, remTime = timeout;
    
	do {
		BQ27XXX_MSLEEP(pollTime);
		ret = bq27xxx_read(di, reg_index, single);
		if (ret < 0)
			break;
		if ((ret & mask)==expctVal || pollTime==0 || timeout==0)
			return (ret & mask);  
		      
		remTime -= pollTime;
	} while (remTime > 0);		// BQ27220 may take up to 1sec to enter full access mode

	dev_err(di->dev, "timeout, reg_index=%d, mask=%#x expctVal=%#x.\n",	reg_index, mask, expctVal);
    return (-EINVAL);
}
...
//    dev_info(di->dev, " *** Read checksum from fuel gauge=%#x\n", ret);    
...
/* TO DO:  Might need modification for your fuel-gauge (tested on BQ27220 only). */
static int bq27xxx_battery_enter_fullaccess_mode(struct bq27xxx_device_info *di)
{
    if (bq27xxx_write(di, BQ27XXX_REG_CTRL, BQ27XXX_FULLACCESS_KEY0, false) >= 0) {
        if (bq27xxx_write(di, BQ27XXX_REG_CTRL, BQ27XXX_FULLACCESS_KEY1, false) >= 0) {
			int expctVal    = BQ27XXX_FULLACCESS_VAL; 	/* Expected value of full-access bit(s)  */
			int bitPosition = BQ27XXX_FULLACCESS_BIT;	/* Bit position of full-access bit(s) (mask) */
			int reg_index   = BQ27XXX_REG_CTRL; 		/* Register containing full-acces flags */

			if (di->chip==BQ27220) {
				expctVal    = BQ27220_FULLACCESS_VAL;
				bitPosition = BQ27220_FULLACCESS_BIT;
				reg_index   = BQ27XXX_REG_OS;
			}
			
            /* Wait to enter full-access mode*/
			return bq27xxx_poll_register(di, reg_index, bitPosition, expctVal, 2000, 10, false);
		}            
	}
	dev_err(di->dev, "failed to enter full-access mode.\n");
    return (-EINVAL);
}
...
static int bq27xxx_battery_cfgupdate_priv(struct bq27xxx_device_info *di, bool active)
{
	...
	int ret, try = limit;
    int reg_index = BQ27XXX_REG_FLAGS;
    int flagbit = BQ27XXX_FLAG_CFGUP;

    if (di->chip == BQ27220) {
        reg_index   = BQ27XXX_REG_OS;
        flagbit     = BQ27220_FLAG_CFGUP;
        if (active)
            cmd = BQ27220_SET_CFGUPDATE;
    }
	...
	do {
		BQ27XXX_MSLEEP(25);
		ret = bq27xxx_read(di, reg_index, false);
		if (ret < 0)
			return ret;
        //dev_info(di->dev, "Waiting to enter CFG_UPDATE mode, retries=%d\n", limit - try);
	} while (!!(ret & flagbit) != active && --try);
					  ...
					  struct bq27xxx_dm_buf *buf)
{
	...
	w = bq27xxx_battery_checksum_dm_block(buf);

    if (di->chip == BQ27220) {
        // The checksum and data length must be written as word (auto increment). Total length is:
        // Total length = BQ27XXX_DM_SZ + 2 bytes addresses + 1 bytes checksum + 1 byte length
        w = w | ((BQ27XXX_DM_SZ+2+1+1) << 8);
        single = false;
    } 
         
    ret = bq27xxx_write(di, BQ27XXX_DM_CKSUM, w, single);
	if (ret < 0)
		goto out;
				       ...
				       struct power_supply_battery_info *info)
{
	...
	// TO DO: Check whether your fuel-gauge requires full access
    if (di->chip == BQ27220) {
		BQ27XXX_MSLEEP(2100);   // Unseal requires at least 2 sec
        if (bq27xxx_battery_enter_fullaccess_mode(di) < 0)
            return;
    }
	...
	if (info->charge_full_design_uah != -EINVAL ||
        ...
        // TODO: Check the assumption below.
		/* assume design energy & capacity are in same block */
		bq27xxx_battery_update_dm_block(di, &bd,
					BQ27XXX_DM_DESIGN_CAPACITY,
					info->charge_full_design_uah / 1000);
        if (di->chip != BQ27220) {   // DESIGN ENERGY is not supported by BQ27220
            bq27xxx_battery_update_dm_block(di, &bd,
                        BQ27XXX_DM_DESIGN_ENERGY,
    ...
    // TERMINATE_VOLTAGE is not supported by BQ27220    
	if (info->voltage_min_design_uv != -EINVAL && di->chip != BQ27220) {
...
/*
 * Return the battery gauge-state register (EDV0, EDV1, EDV2, ...)
 * Or < 0 if something fails.
 *
static int bq27xxx_battery_read_gauge_status(struct bq27xxx_device_info *di)
{
	if (di->chip==BQ27220) {
		if (bq27xxx_write(di, BQ27XXX_REG_CTRL, BQ27220_GAUGE_STATUS, false) >= 0)
			return bq27xxx_read(di, BQ27XXX_DM_DATA, false);			
	}

	dev_err(di->dev, "error reading gauge-status register\n");
	return -ENODATA;
}
*/
 ...
 */
static int bq27xxx_battery_read_operation_status(struct bq27xxx_device_info *di)
{
	int flags;
	bool has_singe_flag = di->opts & BQ27XXX_O_ZERO;

	flags = bq27xxx_read(di, BQ27XXX_REG_OS, has_singe_flag);
	if (flags < 0) {
		dev_err(di->dev, "error reading operation register:%d\n", flags);
		return flags;
	}
	return flags;
}
...
{
	int cyct;
    if ((di->chip==BQ27220) && set_cyclecount)
    {
        cyct = set_cyclecount;
    }
    else
...
static bool bq27xxx_battery_dead(struct bq27xxx_device_info *di, u16 flags)
{
	...
	else if (di->chip==BQ27220)
		return flags & (BQ27220_FLAG_SYSDWN | BQ27220_FLAG_FD);
	else
...
void bq27xxx_battery_update(struct bq27xxx_device_info *di)
{
	...
	if (cache.flags >= 0) {
		...
		if (has_ci_flag && (cache.flags & BQ27000_FLAG_CI)) {
			...
			cache.opr_status = -ENODATA;
		} else {
			if (di->regs[BQ27XXX_REG_OS] != INVALID_REG_ADDR)
				cache.opr_status = bq27xxx_battery_read_operation_status(di);
		...
		if (di->charge_design_full <= 0 || update_design_cap) {
			di->charge_design_full = bq27xxx_battery_read_dcap(di);
            update_design_cap = false;
        }
	}
					  ...
					  union power_supply_propval *val)
{
	...
	if (di->opts & BQ27XXX_O_ZERO) {
		...
		if (di->cache.flags & BQ27XXX_FLAG_FC)
			level = POWER_SUPPLY_CAPACITY_LEVEL_FULL;
		else if (di->cache.opr_status & BQ27220_FLAG_EDV2)
			level = POWER_SUPPLY_CAPACITY_LEVEL_LOW;
...
int bq27xxx_battery_setup(struct bq27xxx_device_info *di)
{
    ...
    if (bq27xxx_chip_data[di->chip].regs==NULL) 
		return -ENODEV;
    
    if (bq27xxx_chip_data[di->chip].dm_regs==NULL) 
		return -ENODEV;
    ...
    if (strncmp(di->name,"bq27426",7)==0)
    {
        psy_desc->name = "BAT1";//di->name; //HACK: LET: to make all fuelgates same driver name
    }
    else
    {
        psy_desc->name = "BAT0";//di->name; //HACK: LET: to make all fuelgates same driver name
    }
