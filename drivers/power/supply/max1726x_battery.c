...
#define MAX1726X_CONFIG2_TALRTEN_MASK   (1 << 6)
...
{
    int res = (int16_t)curr;

    /* Negative */
//    if (res & 0x8000)
//        res |= 0xFFFF0000;

    res *= (int)(1562500 / (priv->pdata->rsense * 1000));
    return res;
}
...
static int max1726x_poll_flag_set(struct regmap *map, int regno, int mask, int timeout)
{
    u32 data;
    int ret;
                   ...
                   union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
        // Design capacity does not change over time and no need to read. Return in uAh
        val->intval = (pdata->designcap >> 1) * 1000; /* DesignCAP LSB: 0.5 mAh */
        break;
    default:
        return -EINVAL;
    }
...
static int max1726x_init(struct max1726x_priv *priv)
{
    ...
    // If design capacity differs from custom data, then the device
    // has not been initialized properly and we have to continue with init.
    regmap_read(regmap, MAX1726X_DESIGNCAP_REG, &reg);  // Read design capacity
    if (reg != pdata->designcap) {
        dev_info(priv->dev, "Design capacity mismatch, forcing SW REST ... ");
        regmap_write(regmap, 0x60, 0x0f);   // Undocumented  Reset command
        regmap_write(regmap, 0xbb, 0x01);   // Undocumented  command

        ret = max1726x_poll_flag_set(regmap, MAX1726X_STATUS_REG, MAX1726X_STATUS_POR, 4000);
        if(ret < 0){
            dev_err(priv->dev, "timeout while waiting for POR flag!\n");
            return ret;
        }
        dev_info(priv->dev, "done\n");
    }
    ...
    dev_info(priv->dev, "Updating fuel gauge registers according to custom data (model %d)\n",pdata->model_option);
...
static struct max1726x_platform_data *max1726x_parse_dt(struct device *dev)
{
    ...
    if (ret)
        pdata->designcap = 0x3458; // 6700 mAh;
