...
/* Returns a boolean indicating whether a B&K base station is connected */
/* Implemented in drivers/bksv/bksv_tcpci.c */
extern enum typec_acc_type tcpci_get_acc_type(struct device *dev);
...
struct lan78xx_net {
	...
	u8 base_station_addr[6];
	bool is_base_station_addr_valid;

	unsigned int usb_error_count;
};
...
/* Returns the number of usb communication errors,
 * used by user-space to decide whether to reset
 * the usb port to get usb comms back on track.
 */
static ssize_t lan78xx_show_usb_error_count(struct device *d,
	struct device_attribute *attr, char *buf)
{
	struct lan78xx_net *dev = netdev_priv(to_net_dev(d));

	return sprintf(buf, "%u\n", dev->usb_error_count);
}

static DEVICE_ATTR(usb_error_count, S_IRUGO, lan78xx_show_usb_error_count, NULL);

static struct attribute *lan78xx_sysfs_usb_attrs[] = {
	&dev_attr_usb_error_count.attr,
	NULL
};

static struct attribute_group lan78xx_sysfs_attr_group = {
	.name = "usb_attrs",
	.attrs = lan78xx_sysfs_usb_attrs,
};

static void inc_usb_error_count(struct lan78xx_net *dev)
{
	dev->usb_error_count++;

	sysfs_notify(&dev->net->dev.kobj, lan78xx_sysfs_attr_group.name, "usb_error_count");
	kobject_uevent(&dev->net->dev.kobj, KOBJ_CHANGE);
}
/* sysfs end */

/* returns true if the LAN7850 is part of the B&K base station */
static bool lan78xx_is_base_station(void)
{
	bool is_base_station;

	/* Get reference to bksv_tcpci driver */
	struct device *dev = bus_find_device_by_name(&i2c_bus_type, NULL, "0-0050");
	if (!dev) {
		printk("bksv_tcpci device not found\n");
		return false;
	}

	is_base_station = tcpci_get_acc_type(dev) == TYPEC_ACC_DEBUG_SOURCE_RP_3_0;
...
/*
 * Read a MAC address from the EEPROM on the digital board.
 * This EEPROM contains a factory programmed MAC address at
 * offset 0x9a.
 */
static int read_mac_addr_eeprom(struct lan78xx_net *dev, u8 *addr)
{
	struct device *eeprom;
	struct i2c_client *client;
	int status, offset = 0x9a, count = 6;

	if (dev->is_base_station_addr_valid) {
		memcpy(addr, dev->base_station_addr, count);
		return 0;
	}

	eeprom = bus_find_device_by_name(&i2c_bus_type, NULL, "2-0058");
	if (!eeprom) {
		return -ENODEV;
	}

	client = container_of(eeprom, struct i2c_client, dev);

	status = i2c_smbus_read_i2c_block_data_or_emulated(client, offset,
		count, dev->base_station_addr);

	put_device(eeprom);

	if (status == count) {
		memcpy(addr, dev->base_station_addr, count);
		dev->is_base_station_addr_valid = true;
		return 0;
	}

	if (status == -ENXIO) {
		/* earlier prototypes did not have this eeprom */
		netdev_info(dev->net, "mac address eeprom not found on digital board");
	} else {
		netdev_err(dev->net, "error %d reading mac address from eeprom on digital board", status);
	}
...
static void lan78xx_init_mac_address(struct lan78xx_net *dev)
{
	...
	netdev_info(dev->net,
		"read %s mac address %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x from lan78xx",
		is_valid_ether_addr(addr) ? "valid" : "invalid",
	...
	if (!is_valid_ether_addr(addr)) {
		...
		if ((lan78xx_read_eeprom(dev, EEPROM_MAC_OFFSET, ETH_ALEN,
			...
			if (is_valid_ether_addr(addr)) {
				...
				netdev_info(dev->net,
					"read mac address %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x from lan78xx eeprom/otp",
					addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]
				);
			} else if (dev->is_base_station && read_mac_addr_eeprom(dev, addr) == 0) {
				netdev_info(dev->net,
					"using b&k base station mac address %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x from digital board eeprom",
					addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]
				);
			} else {
				/* generate random MAC */
				...
				netdev_info(dev->net,
					"using random mac address %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x",
					addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]
				);
			}
		} else if (dev->is_base_station && read_mac_addr_eeprom(dev, addr) == 0) {
...
static int lan78xx_phy_init(struct lan78xx_net *dev)
{
	...
	if (dev->is_base_station) {
		netdev_info(dev->net, "configuring leds for base station");

		/* Configure LED modes:
		 * LED3 off (e)
		 * LED2 off (e)
		 * LED1 link/activity 1000 (1)
		 * LED0 link/activity any speed (0)
		 */
		ret = phy_write(phydev, 29, 0xee10);
			 ...
			 const struct usb_device_id *id)
{
	...
	dev->is_base_station = lan78xx_is_base_station();
	dev->is_base_station_addr_valid = false;
	...
	netdev->sysfs_groups[0] = &lan78xx_sysfs_attr_group;
